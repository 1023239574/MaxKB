...
class ChatSerializers(serializers.Serializer):
    ...
    class Query(serializers.Serializer):
        ...
        def to_row(row: Dict):
            ...
            return [str(row.get('chat_id')), row.get('main_account'), row.get('abstract'), row.get('problem_text'), padding_problem_text,
        ...
        def export(self, with_valid=True):
            ...
            data = [
                ['会话ID', '账号', '摘要', '用户问题', '优化后问题', '回答', '用户反馈', '引用分段数', '分段标题+内容',
    ...
    class OpenChat(serializers.Serializer):
        ...
        main_account = serializers.CharField(required=False, error_messages=ErrMessage.char("煤矿用户账号"))
                           ...
                           ChatInfo(chat_id, chat_model, dataset_id_list,
                                    [str(document.id) for document in
                                     QuerySet(Document).filter(
                                         ...
                                         is_active=False)],
                                    application, main_account), timeout=60 * 30)
            return chat_id
