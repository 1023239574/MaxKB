 ...
 */
const getChatOpen: (application_id: String,main_account: String) => Promise<Result<any>> = (application_id, main_account) => {
  return get(`${prefix}/${application_id}/chat/open?main_account=${main_account}`)
}
/**
 * 对话
 * @param 参数
...
const postChatMessage: (chat_id: string, data: any) => Promise<any> = (chat_id, data) => {
  return postStream(`/damage_gpu/api${prefix}/chat_message/${chat_id}`, data)
}
